...
template <Number T>
struct flat_vector : public std::enable_shared_from_this<flat_vector<T>> {
// Variables    
    T x;
    T y;
...
//  Algebra operators
    ...
    template <Number K>
    friend flat_vector<K> operator* (const K& lambda, const flat_vector<K>& v) noexcept;
...
template <Number T>
bool cmp_by_polar_angle(const flat_vector<T>& a, const flat_vector<T>& b) noexcept;
...
template <Number T>
bool eq_zero(T d, double eps = DOUBLE_EPS_DEFAULT) noexcept;

// Two lines: l1 of a&b, l2 of c&d.
// Throws exception if straight don't cross
template <Number T>
flat_vector<T> intersection(const flat_vector<T>& a,
                         const flat_vector<T>& b,
                         const flat_vector<T>& c,
                         const flat_vector<T>& d);
...
template <Number T>
flat_vector<T> flat_vector<T>::operator+ (const flat_vector<T>& other) const noexcept {
    flat_vector<T> tmp = *this;
    return tmp += other;
}

template <Number T>
flat_vector<T>& flat_vector<T>::operator+= (const flat_vector<T>& other) noexcept {
    this->x += other.x;
    this->y += other.y;
    return *this;
}
...
T angle(const flat_vector<T>& a, const flat_vector<T>& b) noexcept {
    ...
    if (scalar == 0) {
        ...
        return atan2(2.0 * triangle_square(a, b), scalar);
    }
}
template <Number T>
bool cmp_by_polar_angle(const flat_vector<T>& a, const flat_vector<T>& b) noexcept {
...
template <Number T>
flat_vector<T> intersection(
                const flat_vector<T>& a,
                const flat_vector<T>& b,
                const flat_vector<T>& c,
                const flat_vector<T>& d) {
    if (a == b && b == c && c == d) {
