...
TEST(AlgebraOperatorsGroup, FV10_6__MultEq2__EqFV20_12__) {
    flat_vector a(10, 6); int lambda = 2;
...
TEST(CalcNonMemverFuncsGroup, IntersectionConstant) {
    ...
    try {
        flat_vector t = intersection(a, b, c, d);
        EXPECT_TRUE(double_eq_zero(t.x - 3.5));
        EXPECT_TRUE(double_eq_zero(t.y - 2.0));
    } catch(...) {
        FAIL();
    }
}
...
TEST(CalcNonMemverFuncsGroup, IntersectionPoint1) {
    ...
    try {
        flat_vector t = intersection(a, b, c, d);
        EXPECT_TRUE(double_eq_zero(t.x - 6.0));
        EXPECT_TRUE(double_eq_zero(t.y - 4.0));
    } catch(...) {
        FAIL();
    }
}
...
TEST(CalcNonMemverFuncsGroup, IntersectionPoint2) {
    ...
    try {
        flat_vector t = intersection(a, b, c, d);
        EXPECT_TRUE(double_eq_zero(t.x - 4.0));
        EXPECT_TRUE(double_eq_zero(t.y - 4.0));
    } catch(...) {
        FAIL();
    }
